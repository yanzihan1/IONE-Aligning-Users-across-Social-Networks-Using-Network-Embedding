...
import src.openne.IONE_T4_TEST as ONR
from src.openne.IONE_T4_TEST import Data_mask
from .classify import Classifier, read_node_label

class _IONE(object):

    def __init__(self,train_output_file, graph, train_file,rep_size=128, batch_size=1000, negative_ratio=5, order=2):
        self.cur_epoch = 0
        self.order = order
        self.g = graph
    ...
    def build_graph(self,order):
        #h->t   h:parent node vector
        #       t:child input vector
        #h_2    is the value of t. child node vector
        #t_2    is the parent output vector
        ...
        self.look_yp = self.g.look_up_dict
        '''
         如果修改了名字记得修改参数哦！  anchor_emd_file 就是foursquare的output文件, nodes_number是twitter的总节点数目。
         dim是维度
         另外！ 如果数据不是像这个数据集这种 一一对应的，需要修改代码，例如twitter中的20 就是foursquare中的20.
        '''
        dm=Data_mask(train_file=self.train_file,anchor_emd_file=self.train_output_file,nodes_number=5120,dim=100)

        self.mask = tf.convert_to_tensor(dm.mask(self.look_yp))
        a=dm.anc_emd(self.look_yp,order)
        self.anc_emd = tf.convert_to_tensor(dm.anc_emd(self.look_yp,order), dtype=tf.float32)
        ...
        self.embeddings = tf.multiply(self.embeddings, self.mask) + self.anc_emd
        ...
        '''
        self.sign1  ------>x   x_input  x_output   y_input  y_output 
        '''
...
class IONE(object):
    ...
    def __init__(self,train_output_file, graph,train_file, rep_size=128, batch_size=1000, epoch=10, negative_ratio=5, order=3, label_file=None, clf_ratio=0.5, auto_save=True):
        self.rep_size = rep_size
        self.order = order
        self.best_result = 0
        self.vectors = {}
        if order == 3:
            self.model1 = _IONE(train_output_file,graph, train_file,rep_size/2, batch_size,
                                negative_ratio, order=1)
            self.model2 = _IONE(train_output_file,graph,train_file, rep_size/2, batch_size,
            ...
            self.model = _IONE(train_output_file,graph, rep_size, batch_size,
